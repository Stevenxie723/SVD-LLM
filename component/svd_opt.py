...
class SVDOPTAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        r_q = _r.get("q_proj", ratio)
        if r_q != 1:
            q_lr = _lr("q_proj", self.embed_dim, self.embed_dim)
            self.q_u_proj = nn.Linear(q_lr, self.embed_dim, bias=self.enable_bias)
            self.q_v_proj = nn.Linear(self.embed_dim, q_lr, bias=False)
        else:
            self.q_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        r_k = _r.get("k_proj", ratio)
        if r_k != 1:
            k_lr = _lr("k_proj", self.embed_dim, self.embed_dim)
            self.k_u_proj = nn.Linear(k_lr, self.embed_dim, bias=self.enable_bias)
            self.k_v_proj = nn.Linear(self.embed_dim, k_lr, bias=False)
        else:
            self.k_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        r_v = _r.get("v_proj", ratio)
        if r_v != 1:
            v_lr = _lr("v_proj", self.embed_dim, self.embed_dim)
            self.v_u_proj = nn.Linear(v_lr, self.embed_dim, bias=self.enable_bias)
            self.v_v_proj = nn.Linear(self.embed_dim, v_lr, bias=False)
        else:
            self.v_proj = nn.Linear(self.embed_dim, self.embed_dim, bias=self.enable_bias)
        r_out = _r.get("out_proj", ratio)
        if r_out != 1:
            out_lr = _lr("out_proj", self.embed_dim, self.embed_dim)
            self.out_u_proj = nn.Linear(out_lr, self.embed_dim, bias=self.enable_bias)
            self.out_v_proj = nn.Linear(self.embed_dim, out_lr, bias=False)
        else:
...
class SVDOPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig, ratio=1, ratios=None):
        super().__init__()
        self.embed_dim = config.hidden_size

        _attn_ratios = {k: v for k, v in (ratios or {}).items()
                        if k in ("q_proj", "k_proj", "v_proj", "out_proj")}
        self.self_attn = SVDOPTAttention(config=config, ratio=ratio, ratios=_attn_ratios, is_decoder=True)
        ...
        self.ratio = ratio
        _r = ratios or {}
        r_fc1 = _r.get("fc1", ratio)
        if r_fc1 != 1:
            fc1_lr = int(config.ffn_dim * self.embed_dim * r_fc1 / (config.ffn_dim + self.embed_dim))
            self.fc1_u_proj = nn.Linear(fc1_lr, config.ffn_dim, bias=config.enable_bias)
            self.fc1_v_proj = nn.Linear(self.embed_dim, fc1_lr, bias=False)
        else:
            self.fc1 = nn.Linear(self.embed_dim, config.ffn_dim, bias=config.enable_bias)
        r_fc2 = _r.get("fc2", ratio)
        if r_fc2 != 1:
            fc2_lr = int(config.ffn_dim * self.embed_dim * r_fc2 / (config.ffn_dim + self.embed_dim))
            self.fc2_u_proj = nn.Linear(fc2_lr, self.embed_dim, bias=config.enable_bias)
            self.fc2_v_proj = nn.Linear(config.ffn_dim, fc2_lr, bias=False)
        else:
